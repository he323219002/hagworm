...
class Utils():
    ...
    __BYTES_TYPES = (bytes, type(None))

    __STRING_TYPES = (str, type(None))
    ...
    def utf8(cls, val):
        ...
        if isinstance(val, cls.__BYTES_TYPES):
            return val
    ...
    def basestring(cls, val):
        ...
        if isinstance(val, cls.__STRING_TYPES):
            return val
    ...
    @classmethod
    def time2stamp(cls, timestr, format_type=r'%Y-%m-%d %H:%M:%S', timezone=None):

        if(timezone is None):
            return int(datetime.strptime(timestr, format_type).timestamp())
        else:
            return int(datetime.strptime(timestr, format_type).replace(tzinfo=pytz.timezone(timezone)).timestamp())

    @classmethod
    def stamp2time(cls, timeint=None, format_type=r'%Y-%m-%d %H:%M:%S', timezone=None):

        if(timeint is None):
            timeint = cls.timestamp()

        if(timezone is None):
            return time.strftime(format_type, datetime.fromtimestamp(timeint).timetuple())
        else:
            return time.strftime(format_type, datetime.fromtimestamp(timeint, pytz.timezone(timezone)).timetuple())
    ...
    def identity_card(cls, val):
        ...
        if(len(val) == 18):
            ...
            weight_factor = (7, 9, 10, 5, 8, 4, 2, 1, 6,
                             3, 7, 9, 10, 5, 8, 4, 2, 1,)
